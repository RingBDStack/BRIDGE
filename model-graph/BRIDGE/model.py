...
from BRIDGE.models import DGI, GraphCL, Lp, GcnLayers
from BRIDGE.layers import GCN, AvgReadout
import tqdm
import numpy as np
...
class downprompt(nn.Module):
    def __init__(self, weights_list, ft_in, nb_classes, type, feature_dim, num_token=4):
        super(downprompt, self).__init__()
        self.prefeature = prefeatureprompt(
            weights_list, dim=feature_dim, type=type, num_token=num_token
        )
    ...
    def forward(
        ...
        rawret = torch_scatter.scatter(
            src=pretrain_embs1, index=batch, dim=0, reduce="mean"
        )
        rawret = rawret.cuda()
        num = rawret.shape[0]
        if train == 1:
...
class composedtoken(nn.Module):
    def __init__(self, weights_list, type: str, num_tokens=4):
        super(composedtoken, self).__init__()
        self.texttoken = weights_list
        self.prompt = weighted_prompt(num_tokens).cuda()
        self.type = type
...
class prefeatureprompt(nn.Module):
    def __init__(self, weights_list, dim, type: str, num_token=4):
        super(prefeatureprompt, self).__init__()
        self.precomposedfeature = composedtoken(
            weights_list, type, num_tokens=num_token
        )
